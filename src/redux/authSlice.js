...
export const fetchUserProfile = createAsyncThunk(
  ...
  async (tokenOverride, { getState, rejectWithValue }) => {
    try {
      ...
      const token =
        typeof tokenOverride === "string" ? tokenOverride : auth.token;
...
// Async thunk to deposit funds (Updated for Paymob Integration)
export const depositFunds = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      // Updated endpoint: /payments/deposit/ (Paymob flow)
      // Note: Backend URL structure is /api/payments/deposit/
      // API_BASE_URL typically ends with /api
      const response = await fetch(`${API_BASE_URL}/payments/deposit/`, {
        ...
        body: JSON.stringify({
          ...
          user: userId,
          payment_method_id: payment_method_id || null, // Optional for Saved Card
          // transaction_type: 'DEPOSIT' is handled by backend default or not needed for this specific endpoint
        }),
      });
      ...
      if (!response.ok) {
        ...
        if (data) {
          if (typeof data === "string") errorMessage = data;
            ...
            // Extract first error message from dict/list
      ...
      // We do NOT dispatch fetchUserProfile here because the deposit is not complete yet.
      // The user must complete the payment on Paymob first.
      return data; // Returns { iframe_url: ..., ... }
    } catch (error) {
      return rejectWithValue("Network error initiating deposit.");
    }
  }
);
    ...
    try {
      ...
      if (!response.ok) {
        return rejectWithValue(
          data.detail || "Failed to fetch public user profile."
        );
      }
...
export const fetchPublicUsersPaginated = createAsyncThunk(
  ...
  async ({ page = 1, pageSize = 10 }, { rejectWithValue }) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/users/public/all/?page=${page}&page_size=${pageSize}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(register.fulfilled, (state, action) => {
        ...
        const accessToken =
          action.payload.tokens?.access || action.payload.access;
        const refreshToken =
          action.payload.tokens?.refresh || action.payload.refresh;
...
export const { logout, clearError, setSocialLoginData, selectCurrentUser } =
  authSlice.actions;
