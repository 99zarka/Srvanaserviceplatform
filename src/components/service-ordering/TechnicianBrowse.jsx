  ...
  return (
    <div className="container mx-auto p-6" dir="rtl">
      <div className="mb-8 text-center">
        <h1 className="text-4xl font-extrabold text-foreground mb-2">تصفح الفنيين المعتمدين</h1>
        <p className="text-lg text-muted-foreground">ابحث عن الفني المثالي لاحتياجاتك ووظفه مباشرة</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1">
          <Card className="shadow-lg">
            <CardHeader className="bg-secondary text-secondary-foreground rounded-t-lg p-4 border-b border-primary/20">
            ...
            <CardContent className="p-4 space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">بحث سريع</label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pr-10 border-input focus:border-primary focus:ring-primary"
                  />
                </div>
              </div>
              ...
              {/* Location Filter */}
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">الموقع الجغرافي</label>
                <GovernorateSelect
              ...
              {/* Rating Filter */}
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">الحد الأدنى للتقييم</label>
              ...
              {/* Page Size Selector */}
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">عدد الفنيين في الصفحة</label>
        ...
        <div className="lg:col-span-3">
          <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-md text-muted-foreground">
              ...
              <span className="text-sm font-medium text-foreground">الترتيب حسب:</span>
              <Select 
                value={filters.sort_by} 
                onValueChange={(value) => handleFilterChange('sort_by', value)}
              >
                <SelectTrigger className="w-48 border-input focus:border-primary focus:ring-primary">
          ...
          {loading && (!technicians || technicians.length === 0) && (
            <Card className="flex items-center justify-center py-12 col-span-full">
              ...
              <span className="ml-3 text-lg text-foreground">جاري تحميل الفنيين...</span>
            </Card>
          )}
          ...
          {loading && technicians && technicians.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 min-h-96 relative">
              {/* Overlay for loading */}
              <div className="absolute inset-0 bg-background bg-opacity-75 flex flex-col items-center justify-center rounded-lg z-10">
                <Loader2 className="h-10 w-10 animate-spin text-primary mb-4" />
                <span className="text-lg text-foreground">جاري التحميل...</span>
              </div>
          ...
          {!loading && currentTechnicians.length === 0 && (
            <Card className="col-span-full">
              <CardContent className="text-center py-12">
                ...
                <h3 className="text-2xl font-semibold text-foreground mb-3">عذرًا، لم يتم العثور على فنيين</h3>
          ...
          {!loading && currentTechnicians.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {currentTechnicians.map((technician) => (
                <Card key={technician.user_id} className="hover:shadow-lg transition-shadow border-t-4 border-primary/20 rounded-lg overflow-hidden bg-card text-card-foreground">
                  <CardHeader className="p-4 flex flex-col items-center text-center bg-muted/50">
                    <div className="relative w-24 h-24 mb-3 rounded-full overflow-visible flex items-center justify-center bg-muted/50">
                      {technician.profile_photo ? (
                        <img 
                          src={technician.profile_photo} 
                          alt={`${technician.first_name} ${technician.last_name}`} 
                          className="w-full h-full object-cover border-2 border-primary shadow-md rounded-full"
                        />
                      ) : (
                        <User className="h-16 w-16 text-secondary" /> 
                      )}
                      {technician.verification_status === "Verified" && (
                        <CheckCircle className="absolute bottom-[-2px] right-[-2px] h-7 w-7 fill-primary text-primary-foreground stroke-primary-foreground p-0.5 rounded-full bg-primary border border-primary" />
                      )}
                    </div>
                  ...
                  <CardContent className="p-4 space-y-3 border-t border-border bg-muted/20">
                    {/* Rating and Stats */}
                    <div className="flex items-center justify-center gap-4 mb-3">
                      ...
                      <div className="flex items-center gap-1 text-sm font-semibold text-foreground">
                        <div className="flex items-center justify-center w-8 h-8 bg-muted rounded-full">
                          <span className="text-sm font-bold text-foreground">{technician.num_jobs_completed || 0}</span>
                        </div>
                    ...
                    <div className="flex justify-center pt-2">
                      <Button
                        ...
                        className="w-full max-w-xs bg-primary hover:bg-primary/90 text-primary-foreground font-bold py-2 px-4 rounded-md transition-colors"
                      >
                        عرض الملف الشخصي
                      </Button>
                    </div>
          ...
          {/* Pagination Controls */}
          {totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between mt-8 pt-6 border-t border-border">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  className="h-9 w-9 border-input text-foreground hover:bg-accent hover:border-accent"
                >
                    ...
                    if (pageNum === 1 || pageNum === totalPages || (pageNum >= currentPage - 1 && pageNum <= currentPage + 1)) {
                      return (
                        <Button
                          ...
                          className={`w-9 h-9 ${currentPage === pageNum ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'border-input text-foreground hover:bg-accent hover:border-accent'}`}
                        >
                          {pageNum}
                        </Button>
                      );
