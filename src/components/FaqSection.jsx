...
const faqItems = [
  {
    ...
    answer:
      "يعمل فريق دعم سيرفانا من الأحد إلى الخميس من الساعة 9:00 صباحًا حتى 6:00 مساءً، مع إجازة يومي الجمعة والسبت. وفي حال وجود عطلات رسمية، يتم الإعلان عن المواعيد الجديدة على موقعنا الإلكتروني.",
  },
  {
    id: "item-2",
    icon: "credit-card",
              ...
              {faqItems.map((item) => {
                ...
                return (
                  <AccordionItem
                    key={item.id}
                    value={item.id}
                    className="bg-card shadow-sm rounded-lg border px-4 last:border-b"
                  >
